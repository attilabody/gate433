...
{
	if(m_pulseLen && m_cycleLen > m_pulseLen) {
		m_pulseLen = pulseLen;
		m_cycleLen = cycleLen;
	} else
		m_cycleLen = m_pulseLen = 0;

	m_changedAt = m_lastTick = now;
...
void PulsingOutput::Tick(uint32_t now)
{
	...
	if(active && now - m_lastTick >= m_pulseLen) {
		m_changedAt += m_pulseLen;
		HAL_GPIO_TogglePin(m_port, m_pin);
	} else if(!active && now - m_lastTick >= (uint32_t)m_cycleLen - (uint32_t)m_pulseLen) {
